...
namespace
{
	...
	TEST(msgpack, msgpack)
	{
		//TEST(constructors,"individual values")
		{
			...
			//TEST(constructors,"null")
			{
				...
				// roundtrip
				EXPECT_TRUE(json::from_msgpack(result) == j);
				EXPECT_TRUE(json::from_msgpack(result, true, false) == j);
			}

			//TEST(constructors,"boolean")
			{
				//TEST(constructors,"true")
				{
					json j = true;
			...
			{
				//TEST(constructors,"signed")
				{
					//TEST(constructors,"-32..-1 (negative fixnum)")
					{
						for (auto i = -32; i <= -1; ++i)
						{
							//CAPTURE(i);
							...
							EXPECT_TRUE(static_cast<int8_t>(result[0]) == i);
					...
					//TEST(constructors,"0..127 (positive fixnum)")
					{
					...
					//TEST(constructors,"128..255 (int 8)")
					{
					...
					//TEST(constructors,"256..65535 (int 16)")
					{
					...
					//TEST(constructors,"65536..4294967295 (int 32)")
					{
						for (uint32_t i :
								{
					...
					//TEST(constructors,"4294967296..9223372036854775807 (int 64)")
					{
						for (uint64_t i :
								{
					...
					//TEST(constructors,"-128..-33 (int 8)")
					{
					...
					//TEST(constructors,"-32768..-129 (int 16)")
					{
					...
					//TEST(constructors,"-32769..-2147483648")
					{
					...
					//TEST(constructors,"-9223372036854775808..-2147483649 (int 64)")
					{
				...
				//TEST(constructors,"unsigned")
				{
					...
					//TEST(constructors,"128..255 (uint 8)")
					{
						for (size_t i = 128; i <= 255; ++i)
						{
							//CAPTURE(i);
					...
					//TEST(constructors,"256..65535 (uint 16)")
					{
						for (size_t i = 256; i <= 65535; ++i)
						{
							//CAPTURE(i);
					...
					//TEST(constructors,"65536..4294967295 (uint 32)")
					{
						for (uint32_t i :
								{
									65536u, 77777u, 1048576u, 4294967295u
					...
					//TEST(constructors,"4294967296..18446744073709551615 (uint 64)")
					{
						for (uint64_t i :
								{
				...
				//TEST(constructors,"float")
				{
					//TEST(constructors,"3.1415925")
					{
						...
						// roundtrip
						EXPECT_TRUE(json::from_msgpack(result) == j);
						EXPECT_TRUE(json::from_msgpack(result) == v);
						EXPECT_TRUE(json::from_msgpack(result, true, false) == j);
					}
				}
			}
			...
			//TEST(constructors,"string")
			{
				//TEST(constructors,"N = 0..31")
				{
					...
					for (size_t N = 0; N < first_bytes.size(); ++N)
					{
						...
						// check first byte
						EXPECT_TRUE((first_bytes[N] & 0x1f) == N);

						// compare result + size
						const auto result = json::to_msgpack(j);
						EXPECT_TRUE(result == expected);
						EXPECT_TRUE(result.size() == N + 1);
				...
				//TEST(constructors,"N = 65536..4294967295")
				{
					for (size_t N :
							{
			...
			//TEST(constructors,"object")
			{
				...
				//TEST(constructors,"map 16")
				{
					...
					// roundtrip check.
					EXPECT_TRUE(result.size() == 67); // 1 type, 2 size, 16*4 content
					EXPECT_TRUE(result[0] == 0xde); // map 16
					EXPECT_TRUE(result[1] == 0x00); // byte 0 of size (0x0010)
					EXPECT_TRUE(result[2] == 0x10); // byte 1 of size (0x0010)
				...
				//TEST(constructors,"map 32")
				{
					...
					// size and the overall size. The rest is then handled in the
					// roundtrip check.
					EXPECT_TRUE(result.size() == 458757); // 1 type, 4 size, 65536*7 content
					EXPECT_TRUE(result[0] == 0xdf); // map 32
					EXPECT_TRUE(result[1] == 0x00); // byte 0 of size (0x00010000)
					EXPECT_TRUE(result[2] == 0x01); // byte 1 of size (0x00010000)
					EXPECT_TRUE(result[3] == 0x00); // byte 2 of size (0x00010000)
					EXPECT_TRUE(result[4] == 0x00); // byte 3 of size (0x00010000)
		...
		//TEST(constructors,"from float32")
		{
			auto given = std::vector<uint8_t>({0xca, 0x41, 0xc8, 0x00, 0x01});
			json j = json::from_msgpack(given);
			EXPECT_FLOAT_EQ(j.get<double>(), 25.0000019073486);
		}

		//TEST(constructors,"errors")
		{
			//TEST(constructors,"empty byte vector")
			{
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>()), json::parse_error);
				...
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>(), true, false).is_discarded());
			}

			//TEST(constructors,"too short byte vector")
			{
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0x87})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcc})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcd})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcd, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xce})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xce, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xce, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xce, 0x00, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xa5, 0x68, 0x65})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0x92, 0x01})), json::parse_error);
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0x81, 0xa1, 0x61})), json::parse_error);
				...
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0x87}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcc}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcd}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcd, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xce}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xce, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xce, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xce, 0x00, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xcf, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xa5, 0x68, 0x65}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0x92, 0x01}), true, false).is_discarded());
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0x81, 0xA1, 0x61}), true, false).is_discarded());
			}

			//TEST(constructors,"unsupported bytes")
			{
				//TEST(constructors,"concrete examples")
				{
					EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xc1})), json::parse_error);
					...
					EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0xc6}), true, false).is_discarded());

					EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0xc6})), json::parse_error);
				...
				//TEST(constructors,"all unsupported bytes")
				{
					for (auto byte :
							{
								// never used
							...
							})
					{
						...
						EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({static_cast<uint8_t>(byte)}), true, false).is_discarded());
					}
				}
			}

			//TEST(constructors,"invalid string in map")
			{
				EXPECT_THROW(json::from_msgpack(std::vector<uint8_t>({0x81, 0xff, 0x01})), json::parse_error);
				...
				EXPECT_TRUE(json::from_msgpack(std::vector<uint8_t>({0x81, 0xff, 0x01}), true, false).is_discarded());
			}

			//TEST(constructors,"strict mode")
			{
				std::vector<uint8_t> vec = {0xc0, 0xc0};
				//TEST(constructors,"non-strict mode")
				{
				...
				//TEST(constructors,"strict mode")
				{
					...
					EXPECT_TRUE(json::from_msgpack(vec, true, false).is_discarded());
				}
			}
		}

		//TEST(constructors,"SAX aborts")
		{
			//TEST(constructors,"start_array(len)")
			{
				std::vector<uint8_t> v = {0x93, 0x01, 0x02, 0x03};
				SaxCountdown scp(0);
				EXPECT_TRUE(not json::sax_parse(v, &scp, json::input_format_t::msgpack));
			}

			//TEST(constructors,"start_object(len)")
			{
	...
	TEST(msgpack, singleMessagePackroundtrip)
	{
		//TEST(constructors,"sample.json")
		{
			...
			//TEST(constructors,"roundtrips")
			{
				//TEST(constructors,"std::ostringstream")
				{
			...
			EXPECT_TRUE(j1 == json::from_msgpack(packed.begin() + 5, packed.end()));
		}
	}


	TEST(msgpack, MessagePackroundtrips)
	{
		//TEST(constructors,"input from msgpack-python")
		{
			for (std::string filename :
					{
						...
						//"test/data/nst_json_testsuite/test_parsing/y_object_string_unicode.json", //FAILED
					...
					})
			{
				...
				//TEST(constructors,"std::vector<uint8_t>")
				{
					// parse MessagePack file
					std::ifstream f_msgpack(filename + ".msgpack", std::ios::binary);
				...
				//TEST(constructors,"std::ifstream")
				{
					// parse MessagePack file
					std::ifstream f_msgpack(filename + ".msgpack", std::ios::binary);
					json j2;
					EXPECT_NO_THROW(j2 = json::from_msgpack(f_msgpack));

					// compare parsed JSON values
					EXPECT_TRUE(j1 == j2);
				}

				//TEST(constructors,"uint8_t* and size")
				{
					// parse MessagePack file
					std::ifstream f_msgpack(filename + ".msgpack", std::ios::binary);
					std::vector<uint8_t> packed(
						...
						std::istreambuf_iterator<char>());
					json j2;
					EXPECT_NO_THROW(j2 = json::from_msgpack({packed.data(), packed.size()}));

					// compare parsed JSON values
					EXPECT_TRUE(j1 == j2);
				}

				//TEST(constructors,"output to output adapters")
				{
					// parse MessagePack file
					std::ifstream f_msgpack(filename + ".msgpack", std::ios::binary);
					std::vector<uint8_t> packed(
