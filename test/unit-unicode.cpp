...
void check_utf8string(bool success_expected, int byte1, int byte2, int byte3,
                      int byte4);

// create and check a JSON string with up to four UTF-8 bytes
void check_utf8string(bool success_expected, int byte1, int byte2 = -1,
                      int byte3 = -1, int byte4 = -1) {
  if (++calls % 100000 == 0) {
  ...
  if (success_expected) {
    EXPECT_NO_THROW(json::parse(json_string));
  } else {
    EXPECT_THROW(json::parse(json_string), json::parse_error );
  }
}
...
TEST(Unicode, hide) {
   ...
   */
   ...
   {for (int byte1 = 0x80; byte1 <= 0xC1;
         ++byte1){check_utf8string(false, byte1);
}
...
{{for (int byte1 = 0x00; byte1 <= 0x7F; ++byte1){
    ...
    if (0x00 <= byte1 and byte1 <= 0x1F){check_utf8string(false, byte1);
continue;
}
...
{{for (int byte1 = 0xC2; byte1 <= 0xDF;
       ++byte1){for (int byte2 = 0x80; byte2 <= 0xBF;
                     ++byte2){check_utf8string(true, byte1, byte2);
}
}
}
...
{{for (int byte1 = 0xE0; byte1 <= 0xE0;
       ++byte1){for (int byte2 = 0xA0; byte2 <= 0xBF;
                     ++byte2){for (int byte3 = 0x80; byte3 <= 0xBF; ++byte3){
...
{{for (int byte1 = 0xED; byte1 <= 0xED;
       ++byte1){for (int byte2 = 0x80; byte2 <= 0x9F;
                     ++byte2){for (int byte3 = 0x80; byte3 <= 0xBF; ++byte3){
    check_utf8string(true, byte1, byte2, byte3);
}
}
...
{{for (int byte1 = 0xF0; byte1 <= 0xF0;
       ++byte1){for (int byte2 = 0x90; byte2 <= 0xBF;
                     ++byte2){for (int byte3 = 0x80; byte3 <= 0xBF; ++byte3){
    for (int byte4 = 0x80; byte4 <= 0xBF;
         ++byte4){check_utf8string(true, byte1, byte2, byte3, byte4);
}
}
}
}
...

{
...

  {
    ...
    for (std::size_t cp = 0; cp <= 0x10FFFFu; ++cp) {
      ...
      if (cp < 0x10000u) {
        ...
        json_text +=
            codepoint_to_unicode(codepoint1) + codepoint_to_unicode(codepoint2);
      }
  ...
  {
    {
      HELPER_THROW(json::parse("\"\\uDC00\\uDC00\""), json::parse_error,
                   "[json.exception.parse_error.101] parse error at 7: "
                   "syntax error - invalid string: surrogate "
                   "U+DC00..U+DFFF must follow U+D800..U+DBFF; last "
                   "read: '\"\\uDC00'");

      HELPER_THROW(json::parse("\"\\uD7FF\\uDC00\""), json::parse_error,
                   "[json.exception.parse_error.101] parse error at 13: "
                   "syntax error - invalid string: surrogate "
                   "U+DC00..U+DFFF must follow U+D800..U+DBFF; last "
                   "read: '\"\\uD7FF\\uDC00'");

      HELPER_THROW(json::parse("\"\\uD800]\""), json::parse_error,
                   "[json.exception.parse_error.101] parse error at 8: "
                   "syntax error - invalid string: surrogate "
                   "U+DC00..U+DFFF must be followed by U+DC00..U+DFFF; "
                   "last read: '\"\\uD800]'");

      HELPER_THROW(json::parse("\"\\uD800\\v\""), json::parse_error,
                   "[json.exception.parse_error.101] parse error at 9: "
                   "syntax error - invalid string: surrogate "
                   "U+DC00..U+DFFF must be followed by U+DC00..U+DFFF; "
                   "last read: '\"\\uD800\\v'");

      HELPER_THROW(json::parse("\"\\uD800\\u123\""), json::parse_error,
                   "[json.exception.parse_error.101] parse error at 13: "
                   "syntax error - invalid string: '\\u' must be "
                   "followed by 4 hex digits; last read: "
                   "'\"\\uD800\\u123\"'");
...

{
...

  {
    for (auto s : j) {
      ...
      EXPECT_NO_THROW(json::json_pointer("/" + ptr));
...
void roundtrip(bool success_expected, const std::string &s) {
  ...
  if (success_expected) {
    ...
    EXPECT_TRUE(jr.get<std::string>() == s);
  } else {
    // serialization fails
    EXPECT_THROW(j.dump(), json::type_error);

    // parsing JSON text fails
    EXPECT_THROW(json::parse(ps), json::parse_error);
  }
}

TEST(Markus_Kuhns_UTF8_decoder_capability_and_stress_test,test)
{
...
roundtrip(false, "\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c"
...

{
  ...
  roundtrip(false, "\xc0 \xc1 \xc2 \xc3 \xc4 \xc5 \xc6 \xc7 \xc8 \xc9 \xca "
                   "\xcb \xcc \xcd \xce \xcf \xd0 \xd1 \xd2 \xd3 \xd4 \xd5 "
                   "\xd6 \xd7 \xd8 \xd9 \xda \xdb \xdc \xdd \xde \xdf");
  // 3.2.2  All 16 first bytes of 3-byte sequences (0xe0-0xef)
  roundtrip(false, "\xe0 \xe1 \xe2 \xe3 \xe4 \xe5 \xe6 \xe7 \xe8 \xe9 \xea "
                   "\xeb \xec \xed \xee \xef");
...

{
  ...
  roundtrip(false, "\xc0\xe0\x80\xf0\x80\x80\xf8\x80\x80\x80\xfc\x80\x80"
                   "\x80\x80\xdf\xef\xbf\xf7\xbf\xbf\xfb\xbf\xbf\xbf\xfd"
                   "\xbf\xbf\xbf\xbf");
}
